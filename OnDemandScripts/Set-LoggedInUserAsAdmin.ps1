...
Add-PSADTCustom
$CurrentLoggedInUser = [PSADT.QueryUser]::GetSessionInfo('',1)
If ($null -eq $CurrentLoggedInUser) {
    Write-Output "No current logged in user to add to admin group."
} else {
    Write-Output "Adding User [$($CurrentLoggedInUser.NTAccount)] admin group."
    try {
        Add-LocalGroupMember -Group Administrators -Member $CurrentLoggedInUser.NTAccount -ErrorAction Stop
    }
    catch {
        Write-Output "Failed to add account to local admins.  Error: $($_.Exception.Message)"
    }
}
