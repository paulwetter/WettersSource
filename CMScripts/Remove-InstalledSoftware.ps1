$AppNameLike = 'Adobe Acrobat Reader'

#powershell.exe -ExecutionPolicy ByPass -noprofile -NonInteractive -nologo -file .\Uninstall.ps1
...
[pscustomobject[]]$AllApps = Get-InstalledSoftware
$FilteredApp = $AllApps.Where({$_.DisplayName -like "*$AppNameLike*"})
If ($FilteredApp.count -ne 1) {
    exit 1603
}

try {
    $null = Start-Process -FilePath "C:\Windows\System32\MsiExec.exe" -ArgumentList "/x $($FilteredApp.PSChildName) /qn /norestart" -Wait -NoNewWindow -PassThru
}
catch {
    exit 1603
}
